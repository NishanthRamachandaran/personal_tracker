...
const INITIAL_HEALTH_LOGS: HealthLog[] = [
  { id: "hlog-1", user_id: "demo-user-id-001", logged_on: getTodayStr(), water_glasses: 8, sleep_hours: 8.0, workout_minutes: 45, workout_type: "HIIT & Cardio", created_at: new Date().toISOString() },
  { id: "hlog-2", user_id: "demo-user-id-001", logged_on: new Date(Date.now() - 86400000).toISOString().split("T")[0], water_glasses: 7, sleep_hours: 7.5, workout_minutes: 30, workout_type: "Yoga Stretch", created_at: new Date().toISOString() },
];
...
export function useHealthLogs(userId: string) {
  ...
  const healthQuery = useQuery({
    ...
    queryFn: async (): Promise<HealthLog[]> => {
      ...
      if (error || !data || data.length === 0) return INITIAL_HEALTH_LOGS;
  ...
  return {
    healthLogs: healthQuery.data || INITIAL_HEALTH_LOGS,
