...
const INITIAL_MOOD_LOGS: MoodLog[] = [
  { id: "m-1", user_id: DEMO_USER_ID, mood_score: 5, tags: ["Workout", "Focus", "Creative"], journal_note: "Felt super energized during morning deep work!", logged_on: getTodayStr(), created_at: new Date().toISOString() },
  { id: "m-2", user_id: DEMO_USER_ID, mood_score: 4, tags: ["Calm", "Friends"], journal_note: "Nice dinner with close friends.", logged_on: new Date(Date.now() - 86400000).toISOString().split("T")[0], created_at: new Date().toISOString() },
];
...
export function useMoodLogs(userId: string) {
  ...
  return {
    moodLogs: moodQuery.data || (isDemo ? INITIAL_MOOD_LOGS : []),
